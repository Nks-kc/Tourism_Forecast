"""
feature_engineering/validators.py

Validation utilities for the tourism forecasting project.

All validation is performed before preprocessing to ensure that
the dataset is suitable for feature engineering and model training.
...
def validate_duplicate_rows(df: pd.DataFrame) -> List[str]:
    ...
    duplicates = df.duplicated(
        subset=[COUNTRY_COLUMN, DATE_COLUMN]
    )
    ...
    if duplicates.any():
        ...
        errors.append(
            f"Found {count} duplicate country/date rows."
        )
...
def validate_negative_values(df: pd.DataFrame) -> List[str]:
    ...
    if negative.any():
        ...
        errors.append(
            f"Found {count} negative arrival values."
        )
...
def validate_country_names(df: pd.DataFrame) -> List[str]:
    ...
    if invalid.any():
        ...
        errors.append(
            "Dataset contains empty country names."
        )
...
def validate_monthly_sequence(df: pd.DataFrame) -> List[str]:
    ...
    for country, group in grouped:
        ...
        if len(missing):
            ...
            errors.append(
                f"{country}: {len(missing)} missing month(s)."
            )
