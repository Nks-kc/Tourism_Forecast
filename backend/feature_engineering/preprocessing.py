...
Data preprocessing for the tourism forecasting project.
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean and standardize the tourism dataset.
...
def rebuild_date_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Recompute year and month from the date column.
    """
...
def remove_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove duplicate country/date combinations.
    """
